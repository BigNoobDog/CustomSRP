...
CBUFFER_START(_CustomLight)
	...
	float4 _AreaLightXDirections[MAX_OTHER_LIGHT_COUNT];
	float4 _AreaLightYDirections[MAX_OTHER_LIGHT_COUNT];
CBUFFER_END
...
Light GetOtherLight (int index, Surface surfaceWS, ShadowData shadowData) {
	...
	light.areaData = spotAngles;
	light.position = position;
	light.yAxis = _AreaLightYDirections[index].xyz;
	light.xAxis = _AreaLightXDirections[index].xyz;
	return light;
}
