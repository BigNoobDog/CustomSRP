...
namespace MSRP
{
    ...
    {
        PlanarReflectionSettings m_settings;
        
        public GameObject target;
        [FormerlySerializedAs("camOffset")] public float m_planeOffset;

        public static Camera _reflectionCamera;
        private RenderTexture _reflectionTexture;
        private readonly int _planarReflectionTextureId = Shader.PropertyToID("_PlanarReflectionTexture");
        private readonly string _planarReflectionKeywords = "_REFLECTION_PLANARREFLECTION";

        private int2 _oldReflectionTextureSize;

        private PlanarReflectionSettingData data;

        public void SetUp(PlanarReflectionSettings m_settings)
        {
        ...
        public void Cleanup()
        {
            ...
            Shader.DisableKeyword(_planarReflectionKeywords);
        }
        ...
        private void UpdateReflectionCamera(Camera realCamera)
        {
            if (_reflectionCamera == null)
                _reflectionCamera = CreateMirrorObjects(realCamera, m_settings.followZeroPlan ? 0 : 0);
        ...
        private Camera CreateMirrorObjects(Camera realCamera, float height)
        {
            var go = new GameObject("Planar Reflections",typeof(Camera));

            var t = realCamera.transform;
            var reflectionCamera = go.GetComponent<Camera>();
            reflectionCamera.transform.SetPositionAndRotation(new Vector3(t.position.x, height, t.position.z), t.rotation);
        ...
        private void PlanarReflectionTexture(Camera cam, CameraBufferSettings bufferSettings)
        {
            if (_reflectionTexture == null)
            {
                var res = ReflectionResolution(cam, bufferSettings.renderScale);
        ...
        public void ExecutePlanarReflections(ScriptableRenderContext context, Camera camera, CameraBufferSettings bufferSettings)
        {
            ...
            PlanarReflectionTexture(camera, bufferSettings); // create and assign RenderTexture

            data = new PlanarReflectionSettingData(); // save quality settings and lower them for the planar reflections
        ...
        public void SetData()
        {
            ...
            Shader.EnableKeyword(_planarReflectionKeywords);
        }
